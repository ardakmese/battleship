...
def setComputerShips(computer):
    ...
    petrolboat = PatrolBoat("","",True)
    computer.inputShipRandomly(carrier)
    computer.inputShipRandomly(battleship)
    computer.inputShipRandomly(destroyer)
    computer.inputShipRandomly(submarine)
    computer.inputShipRandomly(petrolboat)
...
def checkPosition(size,input):
    ...
    elif not (cols.count(start[0]) == 1 and index.count(int(start[1])) == 1
